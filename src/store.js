...
export function adminView(){ return { categories:db.categories, products:db.products.map(p=>({...p,stock:stock(p.id)})), orders:db.orders.slice(-50).reverse(), topups:db.topups.slice(-50).reverse(), tickets:db.tickets.slice(-50).reverse().map(ticketView), users:Object.values(db.users), audit:db.audit.slice(-50).reverse() }; }
