...
const menu=user=>({inline_keyboard:[[{text:'🛍 Каталог',callback_data:'catalog'},{text:'👤 Кабинет',callback_data:'profile'}],[{text:'📦 Мои покупки',callback_data:'orders'},{text:'💳 Пополнить',callback_data:'topup'}],...(config.admins.has(user.id)?[[{text:'⚙️ Админ-панель',callback_data:'admin'}]]:[])]});
async function show(chatId,user,section,messageId){const data=userView(user);let text='';let keyboard=[];
