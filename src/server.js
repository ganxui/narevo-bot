...
async function verifyAndSettleLztInvoice(invoiceId){
  const invoice=await getLztInvoice(config,invoiceId);
  const status=invoice?.status;
  if(status!=='paid')return {invoice,status,settled:null};
  if(String(invoice.currency||config.lzt.currency).toUpperCase()!==String(config.lzt.currency||'RUB').toUpperCase())throw Error('Валюта счёта LZT Market не совпадает');
  const topupId=invoice.payment_id;
  const settled=settleLztInvoice(topupId,invoice.invoice_id||invoiceId,invoice.amount);
  if(settled.newlyApproved)await notify(settled.userId,`✅ LZT Market подтвердил оплату. Баланс пополнен на <b>${settled.amount} ₽</b>.`);
  return {invoice,status,settled};
}
let lztReconcileRunning=false;
async function reconcilePendingLztTopups(){
  if(lztReconcileRunning||!lztReady(config))return;
  lztReconcileRunning=true;
  try{
    for(const topup of getPendingLztTopups().slice(0,100)){
      try{await verifyAndSettleLztInvoice(topup.lztInvoiceId)}catch(e){console.error(`[LZT reconcile] ${topup.id}: ${e.message}`)}
    }
...
async function reconcilePendingCasheraSbpTopups(){
  if(casheraReconcileRunning||!casheraReady(config))return;
...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(req.url.startsWith('/api/')){const user=auth(req); const admin=(config.demo&&user.id===10001)||await hasAdminAccess(user.id);const seller=admin||(config.demo&&user.id===10001)||await hasSellerAccess(user.id); const b=req.method==='POST'?await body(req):{};
    if(req.url==='/api/me'&&req.method==='GET') return json(res,200,{...userView(user),isAdmin:admin,isSeller:seller});
    if(req.url==='/api/buy'&&req.method==='POST'){const out=buy(user,b.productId,Number(b.quantity||1));await notify(user.id,`✅ Заказ <b>${out.title}</b> на ${out.quantity} шт. выполнен. Товар доступен в разделе «Покупки».`);return json(res,200,out)}
    ...
    if(req.url==='/api/seller'&&req.method==='GET'){if(!seller)return json(res,403,{error:'Нет доступа продавца'});return json(res,200,sellerView(user.id))}
    if(req.url==='/api/seller/products'&&req.method==='POST'){if(!seller)return json(res,403,{error:'Нет доступа продавца'});return json(res,200,addSellerProduct(user.id,b))}
    if(req.url==='/api/seller/stock'&&req.method==='POST'){if(!seller)return json(res,403,{error:'Нет доступа продавца'});return json(res,200,addSellerCodes(user.id,b.productId,b.values||[]))}
    if(req.url==='/api/seller/withdrawals'&&req.method==='POST'){if(!seller)return json(res,403,{error:'Нет доступа продавца'});return json(res,200,createWithdrawal(user.id,Number(b.amount)))}
    ...
    if(req.url==='/api/admin/sellers/assign'&&req.method==='POST')return json(res,200,assignSellerCategory(user.id,b.sellerId,b.categoryId));
    if(req.url==='/api/admin/orders/approve'&&req.method==='POST')return json(res,200,approveSellerOrder(user.id,b.orderId));
    if(req.url==='/api/admin/orders/refund'&&req.method==='POST')return json(res,200,refundSellerOrder(user.id,b.orderId,b.quantity,b.reason));
    if(req.url==='/api/admin/balances'&&req.method==='POST')return json(res,200,adjustUserBalance(user.id,b.userId,b.balanceType,b.mode,b.amount,b.reason));
...
const topupMethodLogLabel=method=>({cryptobot:'CryptoBot',heleket:'Heleket',lzt:'LZT Market',sbp:'СБП',sbp14:'СБП'}[method]||String(method||'Другое'));
...
const linkButton=(text,url,style,icon_custom_emoji_id)=>({text,url,...(style?{style}:{}),...(icon_custom_emoji_id?{icon_custom_emoji_id}:{})});
const pendingInput=new Map();
const broadcastDrafts=new Map();
const sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));

const menu=(user,seller,admin)=>({inline_keyboard:[[button('🛍 Каталог','catalog','primary'),button('👤 Кабинет','profile')],[button('📦 Покупки','orders'),button('💳 Пополнить','topup')],[button('📜 Правила','rules'),button('💬 Поддержка','support')],[button('🔒 Политика конфиденциальности','privacy')],[button('📄 Пользовательское соглашение','terms')],[button('🌐 Язык','language')],...(seller?[[button('🧑‍💼 Панель продавца','seller')]]:[]),...(admin?[[button('⚙️ Админ-панель','admin')]]:[])]});
...
const imageForSection=section=>{let name='catalog';if(section==='home')return config.productImageUrl;if(section==='language')name='language';else if(section==='profile')name='profile';else if(section==='orders'||section.startsWith('order:')||section.startsWith('review:'))name='orders';else if(section==='topup'||section.startsWith('paymethod:')||section.startsWith('topup_custom:'))name='pay';else if(section==='rules'||section.startsWith('rule:'))name='rules';else if(section==='support'||section.startsWith('ticket:'))name='support';else if(section.startsWith('seller'))name='admin';else if(section.startsWith('admin')||section.startsWith('category_')||section.startsWith('product_')||section.startsWith('price_')||section.startsWith('codes_'))name='admin';else if(section==='privacy'||section==='terms'||section==='agree'||section==='welcome')name='rules';return `${config.sectionImageBaseUrl}/section-${name}.png?v=1`};
...
async function hasSellerAccess(userId){if(config.admins.has(userId))return true;if(!config.adminChatId)return false;try{const r=await tgApi('getChatMember',{chat_id:config.adminChatId,user_id:userId});const d=await r.json();if(!d.ok)return false;const member=d.result;return ['creator','administrator','member'].includes(member.status)||(member.status==='restricted'&&member.is_member!==false)}catch{return false}}
...
async function show(chatId,user,section,messageId){
  ...
  else if(section.startsWith('product:')){const p=data.products.find(x=>x.id===section.slice(8));if(!p)return;const categoryIndex=data.categories.findIndex(c=>c.id===p.categoryId);const rating=p.rating?.count?`\nОценка: <b>${p.rating.average}/5</b> · ${p.rating.count} отзывов`:'';text=`<b>📦 ${html(p.title)}</b>\n${html(p.term)}\n\n${html(p.description)}\n\nЦена за 1 шт.: <b>${money(p.price)}</b>\nДоступно: <b>${p.stock}</b>${rating}`;const quantities=[1,2,3,5].filter(q=>q<=p.stock);keyboard=quantities.length?[[...quantities.map(q=>button(`${q} шт.`,`buyqty:${p.id}:${q}`,'primary'))],[button('✏️ Другое количество',`buycustom:${p.id}`)]]:[[button('Нет в наличии','noop')]];keyboard.push([button('← К товарам',`category:${Math.max(0,categoryIndex)}`)]);}
  else if(section.startsWith('buyqty:')){const [,productId,rawQuantity]=section.split(':');const p=data.products.find(x=>x.id===productId);const quantity=Number(rawQuantity);if(!p||!Number.isInteger(quantity)||quantity<1||quantity>p.stock)return;const total=p.price*quantity;text=`<b>Подтверждение покупки</b>\n\n${html(p.title)}\nЦена: ${money(p.price)} × ${quantity}\nИтого: <b>${money(total)}</b>\nБаланс: ${money(data.user.balance)}\nПосле покупки: <b>${money(data.user.balance-total)}</b>`;keyboard=[[button('✅ Подтвердить',`buyconfirm:${p.id}:${quantity}`,'success')],[button('✏️ Изменить количество',`product:${p.id}`)],[button('Отмена','catalog','danger')]];}
  else if(section.startsWith('buycustom:')){const productId=section.slice(10);const p=data.products.find(x=>x.id===productId);if(!p)return;pendingInput.set(user.id,{type:'buy_quantity',productId});text=`<b>Количество товара</b>\n\nВ наличии: ${p.stock}. Отправьте целое число от 1 до ${Math.min(20,p.stock)}.`;keyboard=[[button('Отмена',`product:${p.id}`)]];}
  ...
  else if(section==='orders'){const orders=data.orders.slice(-8).reverse();text='<b>Мои покупки</b>\n\n'+(orders.length?orders.map(o=>`• ${html(o.title)} · ${o.quantity||1} шт.\n${money(o.totalPrice||o.price)} · ${html(o.status)}`).join('\n\n'):'Покупок пока нет.');keyboard=orders.map(o=>[button(`#${o.id.slice(0,8)} · ${o.quantity||1} шт.`,`order:${o.id}`)]);keyboard.push([{text:'‹ В меню',callback_data:'home'}]);}
  else if(section.startsWith('order:')){const order=data.orders.find(o=>o.id===section.slice(6));if(!order)return;const codes=(order.codes||[]).map((code,i)=>`${i+1}. <code>${html(code)}</code>`).join('\n');text=`<b>Заказ #${order.id.slice(0,8)}</b>\n\n${html(order.title)}\nКоличество: ${order.quantity||1}\nЦена: ${money(order.totalPrice||order.price)}\nСтатус: <b>${html(order.status)}</b>\n\n${codes}`;keyboard=[];if(order.sellerId&&['pending_check','approved'].includes(order.status))keyboard.push([button('⚠️ Проблема с заказом',`order_dispute:${order.id}`,'danger')]);if(order.sellerId&&['approved','completed','partially_refunded'].includes(order.status))keyboard.push([button('⭐ Оставить отзыв',`review:${order.id}`,'primary')]);keyboard.push([button('← Мои покупки','orders')]);}
  else if(section.startsWith('review:')){const order=data.orders.find(o=>o.id===section.slice(7));if(!order)return;text=`<b>Отзыв о заказе #${order.id.slice(0,8)}</b>\n\nВыберите оценку:`;keyboard=[[1,2,3,4,5].map(r=>button(`${r} ⭐`,`reviewrate:${order.id}:${r}`,'primary')),[button('← К заказу',`order:${order.id}`)]];}
  ...
  else if(section==='fiat_payments'){const icons=paymentEmojis();text='<b>💳 Банковские карты и СБП</b>\n\nДоступны СБП НСПК и LZT Market.\nМинимальная сумма: 50 ₽';keyboard=[[paymentButton('LZT Market','🟢','paymethod:lzt','success',icons.lzt)],[paymentButton('СБП НСПК · 14%','₽','paymethod:sbp',undefined,icons.sbp)],[button('← Назад','topup')]];}
  ...
  else if(section.startsWith('paymethod:')){const method=section.slice(10);const label=paymentLabel(method);text=`<b>${label}</b>\n\nВыберите сумму пополнения:`;keyboard=[[500,1000,2000].map(x=>button(money(x),`topup:${method}:${x}`,'primary')),[button('✏️ Ввести свою сумму',`topup_custom:${method}`)],[button('← Назад',['sbp','sbp14','lzt'].includes(method)?'fiat_payments':'crypto_payments')]];}
  ...
  else if(section==='seller'&&seller){const s=sellerView(user.id);text=`<b>🧑‍💼 Панель продавца</b>\n\nДоступно к выводу: <b>${money(s.user.sellerAvailableBalance)}</b>\nВ холде 72 часа: <b>${money(s.user.sellerHoldBalance)}</b>\nАктивных товаров: ${s.stats.activeProducts}\nОстаток: ${s.stats.stock}\nЗаказов на проверке: ${s.stats.pendingOrders}\nРейтинг: ${s.stats.rating||'—'} · отзывов ${s.stats.reviewCount}`;keyboard=[[button('📦 Мои товары','seller_products'),button('➕ Добавить товар','seller_add')],[button('📥 Пополнить товар','seller_stock'),button('🧾 Заказы','seller_orders')],[button('⭐ Отзывы','seller_reviews'),button('💰 Баланс и вывод','seller_balance')],[button('📊 Статистика','seller_stats')],[button('← Главное меню','home')]];}
  else if(section==='seller_products'&&seller){const s=sellerView(user.id);text='<b>📦 Мои товары</b>\n\n'+(s.products.length?s.products.map(p=>`${p.moderationStatus==='approved'?'🟢':p.moderationStatus==='pending'?'🟡':'🔴'} ${html(p.title)} · ${p.stock} шт. · ${money(p.price)}`).join('\n'):'Товаров пока нет.');keyboard=s.products.map((p,i)=>[button(`${p.title} · ${p.stock} шт.`,`seller_product:${i}`)]);keyboard.push([button('➕ Добавить товар','seller_add','success')],[button('← Панель продавца','seller')]);}
  else if(section.startsWith('seller_product:')&&seller){const s=sellerView(user.id);const p=s.products[Number(section.slice(15))];if(!p)return;text=`<b>${html(p.title)}</b>\n\nКатегория: ${html(s.categories.find(c=>c.id===p.categoryId)?.title||'—')}\nЦена: ${money(p.price)}\nОстаток: ${p.stock}\nМодерация: ${html(p.moderationStatus)}${p.moderationReason?`\nПричина: ${html(p.moderationReason)}`:''}`;keyboard=p.active?[[button('📥 Пополнить',`seller_stock_product:${p.id}`,'success')]]:[];keyboard.push([button('← Мои товары','seller_products')]);}
  else if(section==='seller_add'&&seller){const s=sellerView(user.id);text='<b>➕ Новый товар</b>\n\nВыберите закреплённую категорию:';keyboard=s.categories.map((c,i)=>[button(`📁 ${c.title}`,`seller_addcat:${i}`)]);if(!s.categories.length)text+='\n\nАдминистратор ещё не закрепил за вами ни одной категории.';keyboard.push([button('← Панель продавца','seller')]);}
  else if(section.startsWith('seller_addcat:')&&seller){const s=sellerView(user.id);const category=s.categories[Number(section.slice(14))];if(!category)return;pendingInput.set(user.id,{type:'seller_product_add',step:'title',draft:{categoryId:category.id}});text=`<b>Новый товар · ${html(category.title)}</b>\n\nОтправьте название товара.`;keyboard=[[button('Отмена','seller_products')]];}
  else if(section==='seller_stock'&&seller){const s=sellerView(user.id);const products=s.products.filter(p=>p.active&&p.moderationStatus==='approved');text='<b>📥 Пополнение товара</b>\n\nВыберите товар:';keyboard=products.map(p=>[button(`${p.title} · ${p.stock} шт.`,`seller_stock_product:${p.id}`)]);if(!products.length)text+='\n\nНет одобренных товаров.';keyboard.push([button('← Панель продавца','seller')]);}
  else if(section.startsWith('seller_stock_product:')&&seller){const productId=section.slice(21);const p=sellerView(user.id).products.find(x=>x.id===productId&&x.active);if(!p)return;pendingInput.set(user.id,{type:'seller_stock',productId});text=`<b>📥 ${html(p.title)}</b>\n\nОтправьте товарные единицы: одна строка — одна единица.`;keyboard=[[button('Отмена','seller_stock')]];}
  else if(section==='seller_orders'&&seller){const s=sellerView(user.id);text='<b>🧾 Заказы продавца</b>\n\n'+(s.orders.length?s.orders.slice(0,12).map(o=>`#${o.id.slice(0,8)} · ${o.quantity||1} шт. · ${money(o.totalPrice||o.price)} · ${html(o.status)}`).join('\n'):'Заказов пока нет.');keyboard=[[button('↻ Обновить','seller_orders')],[button('← Панель продавца','seller')]];}
  else if(section==='seller_reviews'&&seller){const s=sellerView(user.id);const rows=s.reviews.filter(r=>!r.hidden).slice(0,10);text='<b>⭐ Отзывы</b>\n\n'+(rows.length?rows.map(r=>`${'⭐'.repeat(r.rating)}${r.text?`\n${html(r.text)}`:''}`).join('\n\n'):'Отзывов пока нет.');keyboard=[[button('← Панель продавца','seller')]];}
  else if(section==='seller_balance'&&seller){const s=sellerView(user.id);const next=s.orders.filter(o=>o.status==='approved'&&!o.holdReleasedAt).map(o=>o.holdReleaseAt).filter(Boolean).sort()[0];text=`<b>💰 Баланс продавца</b>\n\nДоступно к выводу: <b>${money(s.user.sellerAvailableBalance)}</b>\nВ обязательном холде: <b>${money(s.user.sellerHoldBalance)}</b>\nХолд всегда длится ровно 72 часа и не снимается досрочно.${next?`\nБлижайшее освобождение: ${html(new Date(next).toLocaleString('ru-RU',{timeZone:'Europe/Moscow'}))}`:''}`;keyboard=[[button('Запросить вывод','seller_withdraw','primary')],[button('← Панель продавца','seller')]];}
  else if(section==='seller_withdraw'&&seller){pendingInput.set(user.id,{type:'seller_withdraw'});text='<b>Запрос вывода</b>\n\nВведите сумму от 100 ₽, не превышающую доступный баланс. Заявка будет передана поддержке.';keyboard=[[button('Отмена','seller_balance')]];}
  else if(section==='seller_stats'&&seller){const s=sellerView(user.id);text=`<b>📊 Статистика</b>\n\nПродажи: <b>${money(s.stats.sales)}</b>\nТоваров: ${s.stats.activeProducts}\nОстаток: ${s.stats.stock}\nРейтинг: ${s.stats.rating||'—'}/5\nОтзывов: ${s.stats.reviewCount}`;keyboard=[[button('← Панель продавца','seller')]];}
  else if(section==='admin'&&admin){const a=adminView();text=`<b>NAREVO MAIL · Админ-панель</b>\n\nПользователей: ${a.users.length}\nЗаказов: ${a.orders.length}\nЗаявок: ${a.topups.filter(x=>x.status==='pending').length}\nТикетов: ${a.tickets.filter(x=>x.status==='open').length}\nСтатус бота: <b>${a.botEnabled?'🟢 работает':'🔴 остановлен'}</b>`;keyboard=[[{text:'🎫 Тикеты',callback_data:'admin_tickets'},{text:'💳 Пополнения',callback_data:'admin_topups'}],[{text:'🧑‍💼 Продавцы',callback_data:'admin_sellers'},{text:'🧾 Заказы продавцов',callback_data:'admin_seller_orders'}],[{text:'💸 Выводы',callback_data:'admin_withdrawals'},{text:'💰 Балансы',callback_data:'admin_balances'}],[{text:'📁 Категории',callback_data:'admin_categories'},{text:'📦 Товары и цены',callback_data:'admin_products'}],[{text:'📊 Остатки',callback_data:'admin_stock'}],[{text:'🧾 Лог пополнений · 3 дня',callback_data:'admin_topup_log:0'}],[{text:'📣 Рассылка',callback_data:'admin_broadcast'}],[{text:a.botEnabled?'⏸ Остановить бота':'▶️ Включить бота',callback_data:'admin_toggle_bot',style:a.botEnabled?'danger':'success'}],[{text:'‹ В меню',callback_data:'home'}]];}
  else if(section==='admin_sellers'&&admin){const a=adminView();const sellerIds=[...new Set([...a.sellerAssignments.map(x=>Number(x.sellerId)),...a.products.filter(p=>p.sellerId).map(p=>Number(p.sellerId))])];text='<b>🧑‍💼 Продавцы</b>\n\nВыберите продавца или найдите по Telegram ID:';keyboard=sellerIds.slice(0,15).map(id=>{const u=a.users.find(x=>Number(x.id)===id);return [button(`${u?.name||'Продавец'} · ${id}`,`admin_seller:${id}`)]});keyboard.push([button('🔎 Найти по ID','admin_seller_find','primary')],[button('← Админ-панель','admin')]);}
  else if(section==='admin_seller_find'&&admin){pendingInput.set(user.id,{type:'admin_seller_find'});text='<b>Поиск продавца</b>\n\nОтправьте Telegram ID пользователя.';keyboard=[[button('Отмена','admin_sellers')]];}
  else if(section.startsWith('admin_seller:')&&admin){const sellerId=Number(section.slice(13));const a=adminView();const u=a.users.find(x=>Number(x.id)===sellerId);if(!u)return;text=`<b>Продавец ${html(u.name||'Пользователь')}</b>\n\nID: <code>${sellerId}</code>\nДоступно: ${money(u.sellerAvailableBalance||0)}\nХолд: ${money(u.sellerHoldBalance||0)}\n\nНажмите категорию, чтобы закрепить или снять:`;keyboard=a.categories.map(c=>{const assigned=a.sellerAssignments.some(x=>Number(x.sellerId)===sellerId&&x.categoryId===c.id);return [button(`${assigned?'✅':'▫️'} ${c.title}`,`seller_assign:${sellerId}:${c.id}`)]});const suspended=a.sellerProfiles[String(sellerId)]?.suspended;keyboard.push([button(suspended?'▶️ Восстановить':'⏸ Приостановить',`seller_suspend:${sellerId}`,suspended?'success':'danger')],[button('← Продавцы','admin_sellers')]);}
  else if(section==='admin_seller_orders'&&admin){const orders=adminView().orders.filter(o=>o.sellerId).slice(0,15);text='<b>🧾 Заказы продавцов</b>\n\n'+(orders.length?orders.map(o=>`#${o.id.slice(0,8)} · ${o.quantity||1} шт. · ${money(o.totalPrice||o.price)} · ${html(o.status)}`).join('\n'):'Заказов нет.');keyboard=orders.map(o=>[button(`#${o.id.slice(0,8)} · ${o.status}`,`admin_order:${o.id}`)]);keyboard.push([button('← Админ-панель','admin')]);}
  else if(section.startsWith('admin_order:')&&admin){const o=adminView().orders.find(x=>x.id===section.slice(12));if(!o)return;text=`<b>Заказ #${o.id.slice(0,8)}</b>\n\nПокупатель: <code>${o.userId}</code>\nПродавец: <code>${o.sellerId}</code>\nКоличество: ${o.quantity||1}\nВозвращено: ${o.refundedQuantity||0}\nСумма: ${money(o.totalPrice||o.price)}\nХолд до: ${html(o.holdReleaseAt||'—')}\nСтатус: <b>${html(o.status)}</b>`;keyboard=[];if(['pending_check','disputed','partially_refunded'].includes(o.status)&&!o.approvedAt)keyboard.push([button('✅ Одобрить',`admin_order_approve:${o.id}`,'success')]);if(['pending_check','disputed','partially_refunded','approved'].includes(o.status))keyboard.push([button('↩️ Возврат',`admin_order_refund:${o.id}`,'danger')]);keyboard.push([button('← Заказы','admin_seller_orders')]);}
  else if(section.startsWith('admin_order_refund:')&&admin){const orderId=section.slice(19);pendingInput.set(user.id,{type:'admin_order_refund',orderId});text='<b>Возврат заказа</b>\n\nОтправьте количество единиц для возврата. После этого бот запросит причину.';keyboard=[[button('Отмена',`admin_order:${orderId}`)]];}
  else if(section==='admin_withdrawals'&&admin){const rows=adminView().withdrawals.filter(w=>w.status==='pending').slice(0,15);text='<b>💸 Заявки на вывод</b>\n\n'+(rows.length?rows.map(w=>`#${w.id.slice(0,8)} · ID ${w.sellerId} · ${money(w.amount)}`).join('\n'):'Новых заявок нет.');keyboard=rows.map(w=>[button(`✅ Выплачено ${money(w.amount)}`,`withdraw_paid:${w.id}`,'success'),button('❌',`withdraw_reject:${w.id}`,'danger')]);keyboard.push([button('← Админ-панель','admin')]);}
  else if(section==='admin_balances'&&admin){pendingInput.set(user.id,{type:'admin_balance_user'});text='<b>💰 Балансы пользователей</b>\n\nОтправьте Telegram ID пользователя. Холдинг-баланс вручную не изменяется.';keyboard=[[button('Отмена','admin')]];}
  else if(section.startsWith('admin_balance:')&&admin){const userId=Number(section.slice(14));const u=adminView().users.find(x=>Number(x.id)===userId);if(!u)return;text=`<b>Баланс пользователя</b>\n\n${html(u.name||'Пользователь')} · <code>${userId}</code>\nПокупательский: ${money(u.balance||0)}\nДоступный продавца: ${money(u.sellerAvailableBalance||0)}\nХолд: ${money(u.sellerHoldBalance||0)}\n\nВыберите баланс и действие:`;keyboard=[[button('➕ Покупательский',`balance_edit:${userId}:balance:add`),button('➖ Покупательский',`balance_edit:${userId}:balance:subtract`)],[button('= Покупательский',`balance_edit:${userId}:balance:set`)],[button('➕ Продавца',`balance_edit:${userId}:sellerAvailableBalance:add`),button('➖ Продавца',`balance_edit:${userId}:sellerAvailableBalance:subtract`)],[button('= Продавца',`balance_edit:${userId}:sellerAvailableBalance:set`)],[button('← Админ-панель','admin')]];}
  ...
  else if(section==='admin_products'&&admin){const products=adminView().products.filter(p=>p.active||p.moderationStatus==='pending');text='<b>Товары и цены</b>\n\nВыберите товар:';keyboard=products.map((p,i)=>[{text:`${p.moderationStatus==='pending'?'🟡 ':''}${p.title} · ${money(p.price)}`,callback_data:`admin_product:${i}`}]);keyboard.push([{text:'➕ Добавить товар',callback_data:'product_add'}],[{text:'‹ В админ-панель',callback_data:'admin'}]);}
  ...
  else if(section.startsWith('admin_product:')&&admin){const index=Number(section.slice(14));const a=adminView();const products=a.products.filter(x=>x.active||x.moderationStatus==='pending');const p=products[index];if(!p)return;const category=a.categories.find(c=>c.id===p.categoryId);text=`<b>${html(p.title)}</b>\nКатегория: ${html(category?.title||'Без категории')}\nЦена: ${money(p.price)}\nОстаток: ${p.stock}\nПродавец: ${p.sellerId?`<code>${p.sellerId}</code>`:'магазин'}\nМодерация: ${html(p.moderationStatus||'approved')}`;keyboard=[];if(p.sellerId&&p.moderationStatus==='pending')keyboard.push([button('✅ Одобрить',`seller_product_approve:${p.id}`,'success')],[button('❌ Отклонить',`seller_product_reject:${p.id}`,'danger')]);keyboard.push([button('Изменить цену',`price_edit_id:${p.id}`,'primary')]);if(p.active)keyboard.push([button('Сменить категорию',`product_categories:${index}`,'primary')],[button('Загрузить коды',`codes_add_id:${p.id}`,'success')],[button('Удалить товар',`product_delete_confirm:${index}`,'danger')]);keyboard.push([button('Назад к товарам','admin_products')]);}
...
Страница ${safePage+1}/${pages} · записей: ${rows.length}`;
  ...
  else{text='<b>NAREVO MAIL</b>\nОфициальные цифровые коды подписок.\n\nВыберите раздел:';keyboard=menu(user,seller,admin).inline_keyboard;}
  ...
  if(section.startsWith('admin_topup_log:')||text.length>900){
...
async function botLoop(offset=0){if(!config.token)return;try{const r=await fetch(`https://api.telegram.org/bot${config.token}/getUpdates?timeout=25&offset=${offset}`);const d=await r.json();for(const update of d.result||[]){offset=update.update_id+1;const m=update.message;const q=update.callback_query;
    ...
    if(m?.text&&!m.text.startsWith('/')&&pendingInput.get(m.from.id)?.type==='topup_custom'&&pendingInput.get(m.from.id)?.method==='sbp'){
      try{const amount=Number(m.text.replace(/\s/g,''));const t=await startCasheraSbpTopup(m.from,amount);pendingInput.delete(m.from.id);await tgApi('sendMessage',{chat_id:m.chat.id,text:`<b>Счёт СБП создан</b>\n\n${topupSummary(t)}\n\nПосле оплаты нажмите «Проверить оплату».`,parse_mode:'HTML',reply_markup:casheraSbpKeyboard(t.casheraTransactionUuid,t.paymentUrl)});}catch(e){await tgApi('sendMessage',{chat_id:m.chat.id,text:`Введите сумму ещё раз. ${html(e.message)}`});}continue;
    }
    if(m?.text==='/start'||m?.text==='/menu'){
      ...
      const seller=await hasSellerAccess(m.from.id);
      const response=await tgApi('sendPhoto',{
        chat_id:m.chat.id,
        photo:config.productImageUrl,
        caption:'<b>NAREVO MAIL</b>\nЦифровые коды подписок.\n\nВыберите раздел:',
        parse_mode:'HTML',
        reply_markup:menu(m.from,seller,admin)
      });
    ...
    else if(m?.text==='/emoji'&&await hasAdminAccess(m.from.id)){pendingInput.set(m.from.id,{type:'button_emojis'});await tgApi('sendMessage',{chat_id:m.chat.id,text:'Отправьте <b>одним сообщением четыре премиум-эмодзи</b> в таком порядке:\n\n1. CryptoBot\n2. Heleket\n3. СБП\n4. LZT Market\n\nМожно просто поставить их подряд через пробел.',parse_mode:'HTML'});}
    else if(m?.text==='/lztemoji'&&await hasAdminAccess(m.from.id)){pendingInput.set(m.from.id,{type:'lzt_button_emoji'});await tgApi('sendMessage',{chat_id:m.chat.id,text:'Отправьте <b>один премиум-эмодзи</b>, который нужно поставить на кнопку LZT Market.',parse_mode:'HTML'});}
      ...
      const task=pendingInput.get(m.from.id);const admin=await hasAdminAccess(m.from.id);const seller=await hasSellerAccess(m.from.id);
      try{let done=true;let success='✅ Сохранено. Отправьте /menu, чтобы продолжить.';
        ...
        else if(admin&&task.type==='lzt_button_emoji'){const ids=(m.entities||[]).filter(x=>x.type==='custom_emoji'&&x.custom_emoji_id).map(x=>x.custom_emoji_id);if(ids.length!==1)throw Error('Отправьте ровно один премиум-эмодзи');setLztButtonEmoji(m.from.id,ids[0]);success='✅ Премиум-эмодзи LZT сохранён. Кнопка LZT Market теперь зелёная с этим эмодзи.'}
        ...
        else if(task.type==='buy_quantity'){const quantity=Number(String(m.text).trim());pendingInput.delete(m.from.id);done=false;await show(m.chat.id,m.from,`buyqty:${task.productId}:${quantity}`,getUiMessage(m.chat.id))}
        else if(seller&&task.type==='seller_product_add'){task.draft[task.step]=String(m.text).trim();const next={title:['term','Введите срок или характеристику:'],term:['description','Введите описание товара:'],description:['price','Введите цену за одну единицу:']}[task.step];if(next){task.step=next[0];pendingInput.set(m.from.id,task);done=false;await tgApi('sendMessage',{chat_id:m.chat.id,text:next[1]})}else{const p=addSellerProduct(m.from.id,task.draft);success=`✅ Товар «${p.title}» отправлен на модерацию.`;if(config.adminChatId)await notify(config.adminChatId,`🟡 Новый товар продавца ${m.from.id}: <b>${html(p.title)}</b>`)} }
        else if(seller&&task.type==='seller_stock'){const values=String(m.text).split(/\r?\n/).map(x=>x.trim()).filter(Boolean);if(!values.length)throw Error('Список пуст');pendingInput.set(m.from.id,{type:'seller_stock_confirm',productId:task.productId,values});done=false;await replaceUiWithText(m.chat.id,getUiMessage(m.chat.id),`<b>Подтвердите пополнение</b>\n\nНайдено строк: <b>${values.length}</b>`,{inline_keyboard:[[button('✅ Добавить',`seller_stock_confirm:${task.productId}`,'success')],[button('Отмена','seller_stock','danger')]]})}
        else if(seller&&task.type==='seller_withdraw'){const w=createWithdrawal(m.from.id,Number(String(m.text).replace(/\s/g,'')));success=`✅ Заявка на вывод ${money(w.amount)} создана. Поддержка свяжется с вами.`;if(config.adminChatId)await notify(config.adminChatId,`💸 Новая заявка на вывод ${money(w.amount)} · продавец ${w.sellerId}`)}
        else if(task.type==='review_text'){const review=createReview(m.from.id,task.orderId,task.rating,m.text);success=`✅ Отзыв ${'⭐'.repeat(review.rating)} опубликован.`}
        else if(admin&&task.type==='admin_seller_find'){const sellerId=Number(String(m.text).trim());pendingInput.delete(m.from.id);done=false;await show(m.chat.id,m.from,`admin_seller:${sellerId}`,getUiMessage(m.chat.id))}
        else if(admin&&task.type==='admin_order_refund'){const quantity=Number(String(m.text).trim());if(!Number.isInteger(quantity)||quantity<1)throw Error('Введите целое количество');pendingInput.set(m.from.id,{type:'admin_order_refund_reason',orderId:task.orderId,quantity});done=false;await tgApi('sendMessage',{chat_id:m.chat.id,text:'Введите причину возврата:'})}
        else if(admin&&task.type==='admin_order_refund_reason'){const out=refundSellerOrder(m.from.id,task.orderId,task.quantity,m.text);success=`✅ Возвращено ${out.refundedQuantity} из ${out.quantity} единиц.`;await notify(out.userId,`↩️ По заказу <b>#${out.id.slice(0,8)}</b> оформлен возврат.`);await notify(out.sellerId,`↩️ По заказу <b>#${out.id.slice(0,8)}</b> оформлен возврат.`)}
        else if(admin&&task.type==='admin_balance_user'){const userId=Number(String(m.text).trim());pendingInput.delete(m.from.id);done=false;await show(m.chat.id,m.from,`admin_balance:${userId}`,getUiMessage(m.chat.id))}
        else if(admin&&task.type==='admin_balance_amount'){const amount=Number(String(m.text).replace(/\s/g,''));if(!Number.isFinite(amount)||amount<0)throw Error('Введите корректную сумму');pendingInput.set(m.from.id,{...task,type:'admin_balance_reason',amount});done=false;await tgApi('sendMessage',{chat_id:m.chat.id,text:'Введите обязательную причину изменения:'})}
        else if(admin&&task.type==='admin_balance_reason'){const out=adjustUserBalance(m.from.id,task.userId,task.balanceType,task.mode,task.amount,m.text);success=`✅ Баланс изменён: ${money(out.before)} → ${money(out.after)}.`;await notify(out.userId,`💰 Администратор изменил ваш баланс: ${money(out.before)} → <b>${money(out.after)}</b>.`)}
        else if(task.type==='topup_custom'){const amount=Number(m.text.replace(/\s/g,''));if(task.method==='cryptobot'){const t=await startCryptoBotTopup(m.from,amount);pendingInput.delete(m.from.id);done=false;await tgApi('sendMessage',{chat_id:m.chat.id,text:`<b>Счёт CryptoBot создан</b>\n\nСумма: ${money(t.amount)}\nОплата: USDT\nПосле оплаты вернитесь сюда и нажмите «Проверить оплату».`,parse_mode:'HTML',reply_markup:{inline_keyboard:[[linkButton('Оплатить в CryptoBot',t.paymentUrl,'primary')],[button('✅ Проверить оплату',`cryptocheck:${t.cryptoPayInvoiceId}`,'success')],[button('← Главное меню','home')]]}})}else if(task.method==='heleket'){const t=await startHeleketTopup(m.from,amount);pendingInput.delete(m.from.id);done=false;await tgApi('sendMessage',{chat_id:m.chat.id,text:`<b>Счёт Heleket создан</b>\n\nСумма: ${money(t.amount)}\nПосле оплаты нажмите «Проверить оплату».`,parse_mode:'HTML',reply_markup:heleketKeyboard(t.heleketInvoiceId,t.paymentUrl)})}else if(task.method==='lzt'){const t=await startLztTopup(m.from,amount);pendingInput.delete(m.from.id);done=false;await tgApi('sendMessage',{chat_id:m.chat.id,text:`<b>Счёт LZT Market создан</b>\n\nСумма: ${money(t.amount)}\nПосле оплаты нажмите «Проверить оплату».`,parse_mode:'HTML',reply_markup:lztKeyboard(t.lztInvoiceId,t.paymentUrl)})}else if(task.method==='sbp'){const t=await startCasheraSbpTopup(m.from,amount);pendingInput.delete(m.from.id);done=false;await tgApi('sendMessage',{chat_id:m.chat.id,text:`<b>Счёт СБП создан</b>\n\n${topupSummary(t)}\n\nПосле оплаты нажмите «Проверить оплату».`,parse_mode:'HTML',reply_markup:casheraSbpKeyboard(t.casheraTransactionUuid,t.paymentUrl)})}else{const t=requestTopup(m.from,amount,task.method);success=`✅ Заявка на ${money(t.amount)} создана. Администратор подтвердит пополнение.`;for(const a of config.admins)await notify(a,`💳 Заявка ${task.method} на ${money(t.amount)} · пользователь ${m.from.id}`);if(config.adminChatId)await notify(config.adminChatId,`💳 Заявка ${task.method} на ${money(t.amount)} · пользователь ${m.from.id}`)}}
    ...
    if(q){await tgApi('answerCallbackQuery',{callback_query_id:q.id});const action=q.data;try{
      ...
      if(action.startsWith('buyconfirm:')){const parts=action.split(':');const out=buy(q.from,parts[1],Number(parts[2]));const lines=out.codes.map((code,i)=>`${i+1}. <code>${html(code)}</code>`).join('\n');await tgApi('sendMessage',{chat_id:q.message.chat.id,text:`✅ <b>Покупка выполнена</b>\n${html(out.title)} · ${out.quantity} шт.\n\n${lines}`,parse_mode:'HTML'});if(out.sellerId)await notify(out.sellerId,`🛒 Новый заказ <b>#${out.id.slice(0,8)}</b> · ${out.quantity} шт. · ${money(out.totalPrice)}. Средства помещены в холд на 72 часа.`);await show(q.message.chat.id,q.from,`order:${out.id}`,q.message.message_id)}
      else if(action.startsWith('buy:')){const out=buy(q.from,action.slice(4),1);await show(q.message.chat.id,q.from,`order:${out.id}`,q.message.message_id)}
      else if(action.startsWith('order_dispute:')){const t=openOrderDispute(q.from.id,action.slice(14));if(config.adminChatId)await notify(config.adminChatId,`⚠️ Открыт спор <b>#${t.id}</b> по заказу <code>${t.orderId}</code>.`);await show(q.message.chat.id,q.from,`ticket:${t.id}`,q.message.message_id)}
      else if(action.startsWith('reviewrate:')){const [,orderId,rating]=action.split(':');pendingInput.set(q.from.id,{type:'review_text',orderId,rating:Number(rating)});await tgApi('sendMessage',{chat_id:q.message.chat.id,text:`Вы выбрали ${rating} ⭐. Отправьте комментарий одним сообщением.`})}
      else if(action.startsWith('seller_stock_confirm:')&&await hasSellerAccess(q.from.id)){const task=pendingInput.get(q.from.id);const productId=action.slice(21);if(!task||task.type!=='seller_stock_confirm'||task.productId!==productId)throw Error('Черновик пополнения не найден');const result=addSellerCodes(q.from.id,productId,task.values);pendingInput.delete(q.from.id);await tgApi('sendMessage',{chat_id:q.message.chat.id,text:`✅ Добавлено: ${result.added}\nДубликатов: ${result.duplicates}\nОтклонено: ${result.rejected}`});await show(q.message.chat.id,q.from,'seller_products',q.message.message_id)}
      else if(action.startsWith('seller_assign:')&&await hasAdminAccess(q.from.id)){const [,sellerId,categoryId]=action.split(':');const a=adminView();if(a.sellerAssignments.some(x=>Number(x.sellerId)===Number(sellerId)&&x.categoryId===categoryId))unassignSellerCategory(q.from.id,sellerId,categoryId);else assignSellerCategory(q.from.id,sellerId,categoryId);await show(q.message.chat.id,q.from,`admin_seller:${sellerId}`,q.message.message_id)}
      else if(action.startsWith('seller_suspend:')&&await hasAdminAccess(q.from.id)){const sellerId=Number(action.slice(15));const suspended=Boolean(adminView().sellerProfiles[String(sellerId)]?.suspended);setSellerSuspended(q.from.id,sellerId,!suspended,!suspended?'Приостановлено администратором':'');await show(q.message.chat.id,q.from,`admin_seller:${sellerId}`,q.message.message_id)}
      else if(action.startsWith('seller_product_approve:')&&await hasAdminAccess(q.from.id)){const p=moderateSellerProduct(q.from.id,action.slice(23),true);await notify(p.sellerId,`✅ Товар <b>${html(p.title)}</b> одобрен.`);await show(q.message.chat.id,q.from,'admin_products',q.message.message_id)}
      else if(action.startsWith('seller_product_reject:')&&await hasAdminAccess(q.from.id)){const productId=action.slice(22);const p=moderateSellerProduct(q.from.id,productId,false,'Отклонено администратором');await notify(p.sellerId,`❌ Товар <b>${html(p.title)}</b> отклонён. Обратитесь в поддержку за подробностями.`);await show(q.message.chat.id,q.from,'admin_products',q.message.message_id)}
      else if(action.startsWith('admin_order_approve:')&&await hasAdminAccess(q.from.id)){const o=approveSellerOrder(q.from.id,action.slice(20));await notify(o.userId,`✅ Заказ <b>#${o.id.slice(0,8)}</b> проверен.`);await notify(o.sellerId,`✅ Заказ <b>#${o.id.slice(0,8)}</b> одобрен. Холдинг завершится строго через 72 часа после покупки.`);await show(q.message.chat.id,q.from,`admin_order:${o.id}`,q.message.message_id)}
      else if(action.startsWith('withdraw_paid:')&&await hasAdminAccess(q.from.id)){const w=resolveWithdrawal(q.from.id,action.slice(14),'paid');await notify(w.sellerId,`✅ Выплата ${money(w.amount)} отмечена выполненной.`);await show(q.message.chat.id,q.from,'admin_withdrawals',q.message.message_id)}
      else if(action.startsWith('withdraw_reject:')&&await hasAdminAccess(q.from.id)){const w=resolveWithdrawal(q.from.id,action.slice(16),'rejected','Отклонено администратором');await notify(w.sellerId,`❌ Заявка на вывод ${money(w.amount)} отклонена. Обратитесь в поддержку.`);await show(q.message.chat.id,q.from,'admin_withdrawals',q.message.message_id)}
      else if(action.startsWith('balance_edit:')&&await hasAdminAccess(q.from.id)){const [,userId,balanceType,mode]=action.split(':');pendingInput.set(q.from.id,{type:'admin_balance_amount',userId:Number(userId),balanceType,mode});await tgApi('sendMessage',{chat_id:q.message.chat.id,text:'Введите сумму изменения:'})}
...
Оплатите счёт и повторите проверку.`,parse_mode:'HTML',reply_markup:casheraSbpKeyboard(transaction.uuid||transactionUuid,transaction.payment_url,true)});
      ...
      else if(action.startsWith('price_edit_id:')&&await hasAdminAccess(q.from.id)){const p=adminView().products.find(x=>x.id===action.slice(14));pendingInput.set(q.from.id,{type:'price',id:p?.id});await tgApi('sendMessage',{chat_id:q.message.chat.id,text:'Введите новую цену числом в рублях:'})}
      ...
      else if(action.startsWith('codes_add_id:')&&await hasAdminAccess(q.from.id)){const p=adminView().products.find(x=>x.id===action.slice(13));pendingInput.set(q.from.id,{type:'codes',id:p?.id});await tgApi('sendMessage',{chat_id:q.message.chat.id,text:'Отправьте коды активации: один код на строку.'})}
      else if(action.startsWith('codes_add:')&&await hasAdminAccess(q.from.id)){const p=adminView().products[Number(action.slice(10))];pendingInput.set(q.from.id,{type:'codes',id:p?.id});await tgApi('sendMessage',{chat_id:q.message.chat.id,text:'Отправьте коды активации: один код на строку.'})}
...
server.listen(config.port,()=>{
  ...
  reconcilePendingLztTopups();
  reconcilePendingCasheraSbpTopups();
});
...
const lztReconcileTimer=setInterval(reconcilePendingLztTopups,30_000);
lztReconcileTimer.unref?.();
const casheraReconcileTimer=setInterval(reconcilePendingCasheraSbpTopups,30_000);
casheraReconcileTimer.unref?.();
const sellerHoldTimer=setInterval(()=>{try{releaseMatureHolds()}catch(e){console.error('[Seller hold]',e.message)}},60_000);
sellerHoldTimer.unref?.();
botLoop();
