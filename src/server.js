...
const lztKeyboard=(invoiceId,paymentUrl,repeat=false)=>({inline_keyboard:[...(paymentUrl?[[linkButton('Оплатить через LZT Market',paymentUrl,'primary')]]:[]),[button(repeat?'✅ Проверить ещё раз':'✅ Проверить оплату',`lztcheck:${invoiceId}`,'success')],[button('← Главное меню','home')]]});
...
async function show(chatId,user,section,messageId){
  ...
  else if(section==='fiat_payments'){const icons=paymentEmojis();text='<b>💳 Банковские карты и СБП</b>\n\nДоступны СБП НСПК и LZT Market.\nМинимальная сумма: 50 ₽';keyboard=[[paymentButton('LZT Market','🟢','paymethod:lzt','primary')],[paymentButton('СБП НСПК · 14%','₽','paymethod:sbp',undefined,icons.sbp)],[button('← Назад','topup')]];}
